...
- (void)collectionView {
    ...
    _collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH_PH, SCREEN_HEIGHT_PH - height) collectionViewLayout:layout];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (asset.isCamera) {
        ...
        [PHTool showPhotoCameraInViewController:self sure:^(UIImage *image) {
            ...
            //
            self.publicAsset.cameraImage = nil;
        } dismiss:^{
            if (self.publicAsset.cameraImage) {
                [self cutImageWithImage:self.publicAsset.cameraImage WithStartOrEnd:nil];
            }
        }];
    }
}
...
- (void)backAndGiveValue {
    [PHData imageHighQualityFormatFromPHAsset:self.publicAsset.asset imageSize:PHImageManagerMaximumSize complete:^(UIImage *image) {
        dispatch_async(dispatch_get_main_queue(), ^{
